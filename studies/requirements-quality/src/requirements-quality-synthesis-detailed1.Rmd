---
title: 'Requirements Quality Synthesis: Detailed Review'
author: "Anonymous"
date: '2024-11-11'
...
![Version control view on the evidence evolution](./figs/requirements-quality-synthesis-detailed.png)
...
## Evidence e2: Reanalysis and Revisions
...
All exepct the most important ones (definition of priors and model training) were omitted for brevity.

```{r method-m2-bda}
e2.1.priors <- c(prior(normal(-1, 1), class = Intercept),
            prior(normal(0, 1), class = b))
e2.1 <-
  brm(data = d1, family = binomial, h1.b, prior = e2.1.priors,
...
As such, $e_{2.1}$ still agrees with $e_1$ and $e_{1.1}$ about the significance of passive voice.

### Evidence e2.2a: Revision 1
...
```{r evidence-e2.2a-comparison}
ggcoef_compare(list("Original evidence e1.1" = e1.1, 
                    "First revision e2.2a" = e2.2a), 
...
The conclusion offered by $e_{2.2a}$ consists of the following confidence interval.

```{r evidence-e2.2a}
confint(e2.2a, parm="passiveTRUE", level=0.95)
```

Evidence $e_{2.2a}$ is a bit more cautious about the impact of passive voice on the number of missing associations from a domain model, but remains positive and, hence, agrees with $e_{1.1}$.

### Evidence e2.2b: Revision 2
...
The backdoor adjustment derives the following statistical model from the hypothesis, which produces the evidence $e_{2.2b}$.
...
```{r evidence-e2.2b-conclusion}
confint(e2.2b, parm="passiveTRUE", level=0.95)
```

Evidence $e_{2.2b}$ suggests that passive voice does not have a statistically significant impact on the number of missing associations from domain models.
Comparing the two confidence intervals visualizes this point more clearly.

```{r evidence-e2.2b-cis}
ggcoef_compare(list("First revision e2.2a" = e2.2a,
                    "Second revision e2.2b" = e2.2b))
```
...
### Evidence e2.2c and e2.2d: Revisions 3 and 4

What the follow-up study fails to consider is investigating, why $AIC(e_{2.2a}) \approx AIC(e_{2.2b})$ despite the more rigorous assumptions.
Investigating the $R^2$ values of $e_{2.2b}$ provides some insight.

```{r evidence-e2.2b-r2}
performance::r2(e2.2b)
```

While the marginal $R^2$ value of $e_{2.2b}$ remains low, the conditional $R^2$ value is significantly higher.
...
#### Hypotheses h2c and h2d

In contrast, the hypotheses $h_{2c}$ and $h_{2d}$ discard the factors `exp.academic` and `exp.industrial` and only keep the random effects.
...
h2c.dag <- dagify(
  ...
  exposure = "pv",
  outcome = "masc",
  labels = c(pv="passive.voice", masc="missing.associations", skill="skill", 
             req="req.complexity"),
  coords = list(x=c(pv=0.7, masc=2, skill=0, req=0.3),
                y=c(pv=1, masc=1, skill=1.5, req=2))
)
...
h2d.dag.gg <- ggdag_status(h2d.dag, use_labels = "label",text = FALSE) +
  guides(fill = "none", color = "none") + 
  theme_dag()
```

```{r hypothesed-h2c-h2d}
h2c.dag.gg | h2d.dag.gg
```

These DAGs produce the followin statistical models.

```{r evidence-e2.2c-e2.2d}
h2c <- associations.missing ~ passive + (1|PID) + (1|RID)
e2.2c <- lmer(formula = h2c, data = d1)

h2d <- associations.missing ~ passive + actors.missing + objects.missing + (1|PID) + (1|RID)
e2.2d <- lmer(formula = h2d, data = d1)
```

#### Diagnostics

The AIC values of both $e_{2.2c}$ and $e_{2.2d}$ are significantly lower than that of $e_{2.2b} (251.2111).

```{r evidence-e2.2c-e2.2d-aic}
AIC(e2.2c)
AIC(e2.2d)
```

Consequently, $h_{2d}$ has the greatest external validity and should have been used for further analyses.

#### Conclusion

The conclusions offered by the two pieces of evidence $e_{2.2c}$ and $e_{2.2d}$ look as follows.

```{r evidence-e2.2c-e2.2d-conclusion}
ggcoef_compare(list("e2.2c" = e2.2c,
                    "e2.2d" = e2.2d))
```

The impact of passive voice is considered significant again.
Additionally, $e_{2.2d}$ shows a significant mediatio via the factor `objects.missing`.
...
pos <- position_nudge(y = ifelse(cmb$model == "e2.1", -0.1, 0.1))
...
### Evidence e2.3: Reanalysis

According to the revisions $e_{2.2a}$, $e_{2.2b}$, $e_{2.2c}$, and $e_{2.2d}$, the most valid hypothesis would have been $h_{2d}$, not $h_2$ that was involved in producing $e_2$.

#### Method m2: Bayesian data analysis

```{r hypothesis-h2d-bayesian}
h2d.b <- (associations.missing | trials(associations.expected) ~ 
           passive + actors.missing + objects.missing +
           (1|RID) + (1|PID))
```

```{r evidence-e2.3, message=FALSE, warning=FALSE}
e2.3 <-
  brm(data = d1, family = binomial, h2d.b, prior = e2.priors,
    iter = 4000, warmup = 1000, chains = 4, cores = 4,
    seed = 4,
    file = "fits/e2.3"
  )
...
It has already been proven that $h_{2d}$ is more externally valid than $h_2$.
However, we can compare the two Bayesian models using the leave-one-out comparison.

```{r evidence-e2.3-loo}
e2 <- add_criterion(e2, "loo")
e2.3 <- add_criterion(e2.3, "loo")
loo_compare(e2, e2.3)
```

Again, $e_{2.3}$ shows the better out-of-sample predictive power, confirming that $h_{2d}$ should have been used over $h_2$.

#### Conclusion

Finally we can inspect the conclusion of $e_{2.3}$.

```{r evidence-e2.3-comaprison}
# obtain the parameters from the posterior predictions
params.m1 <- mcmc_intervals_data(e2)
params.m2 <- mcmc_intervals_data(e2.3)

# add the "model" variable to each row in the parameter list
params.m1$model <- "e2"
params.m2$model <- "e2.3"
...
Evidence $e_{2.3}$ is still consistent with 0 but only by a slight margin.

```{r evidence-e2.3-conclusion}
evaluate.model(e2.3)
```
