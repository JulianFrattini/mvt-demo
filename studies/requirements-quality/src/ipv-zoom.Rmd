...
Using a linear mixed model ($m_{1.2}$) instead of a linear model ($m_{1.1}$) technically constitutes a reanalysis.
However, we can consider $e_{1.1}$ produced by a linear mixed model without a random effect as long as its residuals are independent and identically distributed.
...
Technically, this impedes the comparability of $m_{1.1}$ and $m_{1.2}$.
However, we assume that we can compare the linear model $e_{1.1}$ with the linear mixed model $e_{1.3b}$ as a version without any random effect.
