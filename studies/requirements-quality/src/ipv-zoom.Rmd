...
The AIC value of $e_{2.2c}$ is significantly lower than that of $e_{2.2b} (251.2111).
...
The conclusion offered by evidence $e_{2.2c}$ looks as follows.

```{r evidence-e2.2c-conclusion}
summary(e2.2c)
```

```{r evidence-e2-2c-passive}
confint(e2.2c, parm="passiveTRUE", level=0.95)
```

$e_{2.2c}$ considers the impact of passive voice to be significant again.
Additionally, $e_{2.2c}$ shows a significant mediation via the factor `objects.missing`.
